...
//                String userID="bd52568e0afb26b383dd14eeb105a6bb";
...
//
//                                    // Extract the values from storage
//                                    BigInteger received_Mk_3;
//                                    try {
//                                        // Extract the value as a string to prevent any floating-point conversion
//                                        //jsonMsg4Data.getString("Mk_4")
//                                        String mu_i_str = jsonMsg4Data.get("Mk_3").toString();
//                                        received_Mk_3 = new BigInteger(mu_i_str);  // Convert directly to BigInteger
//                                        System.out.println("mu_i (as BigInteger): " + received_Mk_3);
//                                    } catch (NumberFormatException e) {
//                                        System.err.println("Error while converting received_Mk_3 to BigInteger: " + e.getMessage());
//                                        e.printStackTrace();
//                                        return;
//                                    }
//
...
//                                    SK = "3130abb46ca183e2f4d36b27d5a9031712668a0e1dc43c70d19c0ee76712aeca";
//
...
//                                        intent.putExtra("port", 11111); // Assuming this is the default port for streaming
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String KEM_NAME = "Kyber512";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            // Generate CRYSTALS-Kyber keys using liboqs-java
            KeyEncapsulation keyEncapsulation = new KeyEncapsulation(KEM_NAME);
            Pair<byte[], byte[]> keyPair = keyEncapsulation.generate_keypair();
            byte[] publicKey = keyPair.getLeft();
            byte[] secretKey = keyPair.getRight();

            // Log the generated keys
            Log.d(TAG, "Generated Public Key: " + bytesToHex(publicKey));
            Log.d(TAG, "Generated Secret Key: " + bytesToHex(secretKey));

            // Encode the shared key using Base64 to send via Intent
            String encodedKey = Base64.encodeToString(secretKey, Base64.DEFAULT);
            Log.d(TAG, "Encoded Shared Key (Base64): " + encodedKey);

            // Send the encoded key and other data to the VideoStreamActivity
            Intent intent = new Intent(MainActivity.this, VideoStreamActivity.class);
            intent.putExtra("sharedKey", encodedKey);  // Sending Base64 encoded key as a string
            intent.putExtra("serverAddress", "10.0.2.2");
            intent.putExtra("port", 11111);
            startActivity(intent);
        } catch (Exception e) {
            Log.e(TAG, "Error generating keys: " + e.getMessage(), e);
        }
    }
